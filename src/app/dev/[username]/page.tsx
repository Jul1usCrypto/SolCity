...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!dev) {
    return { title: "Developer Not Found - SolCity" };
  }

  const contribs = (dev.contributions_total && dev.contributions_total > 0) ? dev.contributions_total : dev.contributions;
  const title = `@${dev.github_login} - SolCity | ${contribs.toLocaleString()} contributions`;
  const description = `See @${dev.github_login}'s building in SolCity. ${contribs.toLocaleString()} contributions, ${dev.public_repos.toLocaleString()} repos, ${dev.total_stars.toLocaleString()} stars. Rank #${dev.rank ?? "?"} in the city.`;
...
export default async function DevPage({ params }: Props) {
  ...
  const breadcrumbJsonLd = {
    ...
    "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "SolCity", item: baseUrl },
      {
        "@type": "ListItem",
        position: 2,
