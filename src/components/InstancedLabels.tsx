...
function createTextAtlas(buildings: CityBuilding[]): THREE.CanvasTexture {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Show name if available; shorten wallet-like addresses (base58, 32+ chars)
    let displayLabel: string;
    if (b.name && b.name.length > 0 && !b.name.startsWith("filler_")) {
      displayLabel = b.name.length > 14 ? b.name.slice(0, 14).toUpperCase() : b.name.toUpperCase();
    } else if (b.login.length >= 32 && /^[1-9A-HJ-NP-Za-km-z]+$/.test(b.login)) {
      displayLabel = `${b.login.slice(0, 4)}...${b.login.slice(-4)}`.toUpperCase();
    } else if (b.login.length > 16) {
      displayLabel = b.login.slice(0, 16).toUpperCase() + "...";
    } else {
      displayLabel = b.login.toUpperCase();
    }
