...
export default function BuildingAds({ ads, buildings, onAdClick, onAdViewed, focusedBuilding, focusedBuildingB }: BuildingAdsProps) {
  ...
  // Top 10 tallest buildings for billboards & LED wraps
  const top10 = useMemo(
    () =>
      [...buildings]
  ...
  // Scattered medium-high buildings for signs & billboards (skip top10, pick every Nth)
  const scatteredBuildings = useMemo(() => {
    const sorted = [...buildings].sort((a, b) => b.height - a.height);
    const top10Logins = new Set(sorted.slice(0, 10).map(b => b.login));
    // Pick from buildings ranked 11+ (medium-high), spread evenly
    const candidates = sorted.filter(b => !top10Logins.has(b.login) && b.height > 15);
    if (candidates.length === 0) return sorted.slice(0, 20); // fallback
    const maxPick = 20;
    const stride = Math.max(1, Math.floor(candidates.length / maxPick));
    const picked: typeof candidates = [];
    for (let i = 0; i < candidates.length && picked.length < maxPick; i += stride) {
  ...
  return (
    <group ref={groupRef}>
      {billboardAds.map((ad, i) => {
        // First 60% on KOL/top buildings, rest scattered across the city
        const kolCount = Math.ceil(billboardAds.length * 0.6);
        const building = i < kolCount
          ? top10[i % top10.length]
          : scatteredBuildings[(i - kolCount) % scatteredBuildings.length];
      ...
      {rooftopSignAds.map((ad, i) => {
        // First half on top/KOL buildings, second half scattered across the city
        const half = Math.ceil(rooftopSignAds.length / 2);
        const building = i < half
          ? top10[i % top10.length]
          : scatteredBuildings[(i - half) % scatteredBuildings.length];
        if (!building) return null;
        const loginLower = building.login.toLowerCase();
        const isDimmed = !!focusedLower && loginLower !== focusedLower && loginLower !== focusedBLower;
        return (
